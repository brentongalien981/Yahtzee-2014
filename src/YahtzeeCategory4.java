/*
 *	This is the final version of checking categories.
 *	This has a run() method for testing.
 */
import acm.program.*;

public class YahtzeeCategory4 extends ConsoleProgram implements YahtzeeConstants {
	private static boolean test;
	private int dice[] = new int[5];
	private int categ;
	private static String fromCateg;
	...
	public void run() {
		while (true) {			
			...
			categ = readInt("category: ");
			
			checkCategory(dice, categ);
			print(fromCateg + ": ");
			println(test);
		}
	}
	...
	public static boolean checkCategory(int dice[], int category) {
		...
		if (category <= SIXES || category == CHANCE) { test = true; fromCateg = "Random"; }
		else {
			switch(category) {
			case THREE_OF_A_KIND:
				threeOfAKind(dice);
				fromCateg = "3 of A Kind";
				break;
			case FOUR_OF_A_KIND:
				fourOfAKind(dice);
				fromCateg = "4 of A Kind";
				break;
			case FULL_HOUSE:
				fullHouse(dice);
				fromCateg = "Full House";
				break;
			case SMALL_STRAIGHT:
				smallStraight(dice);
				fromCateg = "Small Straight";
				break;
			case LARGE_STRAIGHT:
				largeStraight(dice);
				fromCateg = "Large Straight";
				break;
			case YAHTZEE:
				yahtzee(dice);
				fromCateg = "Yahtzee";
